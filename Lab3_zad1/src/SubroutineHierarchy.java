...
public class SubroutineHierarchy extends JPanel {
	...
	//	(Graphics2D g2,double skala_x , double skala_y, 
	...
	{ 
		...
		int N = 16;
        for (int i = 0; i < N; i++ ) {
        	Shape line = new Line2D.Float((float)Math.sin(2*Math.PI/N*i)*2, (float)Math.cos(2*Math.PI/N*i)*2, (float)Math.sin(2*Math.PI/N*(i+1))*2, (float)Math.cos(2*Math.PI/N*(i+1))*2);
        	g2.draw(line);
        }
		//szprychy
        for (int i = 0; i < (N/2); i++ ) {
        	Shape line = new Line2D.Float((float)Math.sin(2*Math.PI/N*i)*2, (float)Math.cos(2*Math.PI/N*i)*2, (float)Math.sin(2*Math.PI/N*(i+8))*2, (float)Math.cos(2*Math.PI/N*(i+8))*2);
        	g2.draw(line);
        }
